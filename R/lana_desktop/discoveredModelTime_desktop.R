buildVariantTimedDesktopFilterSettings <- function(logId, startDate, endDate){
  rqBody <- paste0('
       {
        ...
        "traceFilterSequence": [
          {
            "from": ', as.character(as.integer(as.POSIXct(startDate))) ,'000,
            "to": ', as.character(as.integer(as.POSIXct(endDate))) ,'000,
            "startInRange": false,
            "endInRange": true,
            "inverted": false,
...
#' @name timeFilter
timeFilter_desktop <- function(logName, startDate, endDate, type="timeRangeFilter"){

  logId <- lanar::chooseLog(logName)

  rqBody <- lanar::buildVariantTimedFilterSettings(logId, startDate, endDate)
